...
Phase 6.6: 完整配置 key 幂等, max_bytes/backup_count, 禁止 traceback 泄露。
"""
...
_REDACT_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"secret[_-]?key\s*[=:]\s*\S{10,}", re.IGNORECASE), "secret_key=[REDACTED]"),
...
def setup_logging(
    ...
    相同配置重复调用不叠加 handler；配置变化自动重配。
    """
    global _current_config
    logger = logging.getLogger("project_memory_mcp")

    new_config = (str(log_dir) if log_dir else None, level, enable_file, enable_stderr, max_bytes, backup_count)
    if not force and _current_config == new_config:
        return logger
